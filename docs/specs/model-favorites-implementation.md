...
最终完整运行（交互精简及恢复点增补后）：服务端 389 项通过，Web 179 项通过；typecheck、lint、Biome 和 Web build 退出码均为 0。
...
- 预览默认显示工具、模型、操作方式、写入文件及告警；参数和原生文件差异按需展开。
...
- 恢复点由本机 AES 密钥加密，落盘权限 0600，API 仅返回时间、来源和 ID。它依赖当前用户目录及本机密钥，不是异机灾备包；异机迁移继续使用现有加密导出。
