...
## 组合工作台与时间线（2026-09-05）

- 常驻一级导航统一为工作台、模型收藏、恢复时间线。工作台以工具为起点，只展示当前已经生效的真实配置链路；“配置与切换”在一个工作区中收敛已有档案与收藏模型两种来源。选择收藏后，当前链路和未写入的候选链路以横向二叉树汇聚到同一个工具节点，再进入原生文件预览；歧义渠道需明确选择。
- 根据侧栏反馈，工作台恢复原有 17rem 紧凑工具栏，沿用图标、当前档案和数量提示；模型切换与关系展示放在右侧内容区。工作台与详细配置复用同一导航组件和工具选择，返回时保留当前工具。
- 收藏页使用模型列表与供应商→模型→工具关系视图，工具状态基于实际关联与选定渠道。手动添加继续复用统一模型选择器；从当前配置收藏预填来源与名称，提取凭据仍需明确同意。
- 全局恢复改为时间线、快照层次和文件影响预览。选择恢复点只读，确认使用对应指纹；快速切换恢复点时旧请求不能覆盖新预览。离开切换页面后过时计划也不会回填。
- 桌面短窗口的时间点、文件影响独立滚动，恢复范围和确认按钮保留在快照内；明暗主题及减少动态效果共用现有设计体系。
- 最新自动化结果：服务端 395 项、Web 193 项通过；typecheck、lint、Biome、生产构建通过。实机已验证工作台到原生文件预览、收藏关系页和恢复点预览，没有执行真实激活或恢复。
- 最终浏览器复查被 CUA 连接中断阻挡，短窗口布局修正及紧凑侧栏尚未复拍，详见 [设计验收记录](../../design-qa.md)。PR 保持草稿；这些自动化结果不代表视觉验收及五种 CLI 发布验收已经完成。
