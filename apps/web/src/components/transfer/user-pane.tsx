...
/**
 * Copies another local account's profiles and the credentials they reference.
 *
 * Conflicts are resolved per harness rather than by one global policy: the accounts on a
 * box usually overlap in some tools and not others, so a single switch would be too blunt.
 */
export function UserPane({ onDone }: { onDone: () => void }) {
    ...
    <div className="space-y-5">
      ...
      {preview ? (
        <div className="space-y-4 rounded-2xl border bg-muted/25 p-4">
          ...
          <p className="text-xs leading-relaxed text-muted-foreground">{t('sync.storeOnlyHint')}</p>
        </div>
      ) : null}
